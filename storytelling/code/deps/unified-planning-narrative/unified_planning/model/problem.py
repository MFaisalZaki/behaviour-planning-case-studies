...
class Problem(  # type: ignore[misc]
    ...
    def __init__(
        ...
        *,
        ...
        self._intendable_fluents:  List["up.model.fnode.FNode"] = list()
        self._delegatable_fluents: List["up.model.fnode.FNode"] = list()

        self._negated_intendable_fluents:  List["str"] = list()
        self._negated_delegatable_fluents: List["str"] = list()
    ...
    def __repr__(self) -> str:
        ...
        if len(self.quality_metrics) > 0:
            ...
            s.append("]\n\n")
        if len(self._intendable_fluents) > 0:
            s.append("Intendable fluents =[\n")
            s.extend(map(custom_str, self._intendable_fluents))
            s.append("]\n\n")
        if len(self._delegatable_fluents) > 0:
            s.append("Delegatable_fluents fluents =[\n")
            s.extend(map(custom_str, self._delegatable_fluents))
            s.append("]\n\n")
        return "".join(s)
    ...
    def add_intendable_fluent(self, fluentname:str) -> None:
        # search for the fluent that has f and add it to the list.
        # Check if the fluent has a not then create a not fluent for it.
        if fluentname.startswith('not-'): self._negated_intendable_fluents.append(fluentname)
        self._intendable_fluents.extend(filter(lambda f: f.name == fluentname, self._fluents))

    def add_delegatable_fluent(self, fluentname:str) -> None:
        # search for the fluent that has f and add it to the list.
        if fluentname.startswith('not-'): self._negated_delegatable_fluents.append(fluentname)
        self._delegatable_fluents.extend(filter(lambda f: f.name == fluentname, self._fluents))

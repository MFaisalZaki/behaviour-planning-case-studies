...
from unified_planning.model import ContingentProblem, OperatorKind
...
class PDDLGrammar:
    def __init__(self):
        ...
        domain = (
            ...
            + Optional(types_def)
            + Optional(intendable_def)
            + Optional(delegatable_def)
...
class PDDLReader:
        ...
        while len(stack) > 0:
            ...
            if status:
                ...
                if isinstance(exp.value, ParseResults):
                    ...
                    # Handle belief
                    elif exp[0].value in ["believes", "intends", "delegated"]:
                        parameter_obj = up.model.parameter.Parameter #(param_name, param_type, self._env)
                        actor = self._parse_exp(problem, act, types_map, var, exp[1], complete_str)
                        fact  = self._parse_exp(problem, act, types_map, var, exp[2], complete_str)
                        # params = OrderedDict()
                        # params[str(actor)] = actor.type
                        # params['fact'] = fact.type
                        # params = [actor, fact]
                        # TODO: Recheck if failed.
                        params  = [parameter_obj(f'actor-{str(actor)}', actor.type, self._env)]
                        params += [parameter_obj('fact', fact.type, self._env)]

                        # params = [parameter_obj('bool' if a.node_type == OperatorKind.BOOL_CONSTANT else a.type.name, a.type, a.environment) for a in [actor, fact]]
                        fluent = up.model.Fluent(exp[0].value, self._tm.BoolType(), params, self._env)
                        problem.add_fluent(fluent)
                        solved.append(self._em.FluentExp(fluent, (actor, fact)))
                    elif exp[0].value in ["justified"]:  # quantifier operators
                        assert False, "justified: Not implemented"
                    elif exp[0].value in ["exists", "forall"]:  # quantifier operators
    ...
    def _add_fail(
        ...
        while to_add:
            ...
            if op == "and":
                ...
                act.add_fail(*eff if timing is None else (timing, *eff), forall=tuple(forall_variables.values()))  # type: ignore
            elif op == "assign":
                eff = (
                    self._parse_exp(
                        problem, act, types_map, forall_variables, exp[1], complete_str
    ...
    def _add_actors(
        ...
        for i, p in enumerate(exp.value[0]):
            try:
                act.add_agent(self._em.ParameterExp(act.parameter(p)))
            except ValueError:
                pass
    ...
    def _parse_problem(
        ...
        for intend in domain_res.get("intendable", []):
            for fluentname in [p for p in intend.value[0]]:
                problem.add_intendable_fluent(fluentname)

        for delegate in domain_res.get("delegatable", []):
            for fluentname in [p for p in delegate.value[0]]:
                problem.add_delegatable_fluent(fluentname)
